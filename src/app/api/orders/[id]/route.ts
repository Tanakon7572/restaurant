...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const menu = menuItemIds.length > 0 ? await loadMenuForPricing(prisma, menuItemIds, false, { staff: true }) : new Map()
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (body.items) {
      ...
      const menu = await loadMenuForPricing(prisma, inputs.map(i => i.menuItemId), false, { staff: true })
